...
const ACCESS_SECRET: string = getSecret("JWT_SECRET");
const REFRESH_SECRET: string = getSecret("REFRESH_TOKEN_SECRET");
...
export function signRefreshToken(tenantId: number, userId?: number): { token: string; jti: string } {
  ...
  const token = jwt.sign(payload, REFRESH_SECRET, { expiresIn } as jwt.SignOptions);
  return { token, jti };
}
