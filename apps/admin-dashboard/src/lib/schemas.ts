...
export const programSchema = z.object({
    ...
    min_salary: z.number().nonnegative("Must be ≥ 0"),
    max_customer_age: z.number().int().positive("Must be > 0"),
    ...
    interest_rate: z.number().min(0).max(100, "Must be between 0 and 100"),
    profit_rate: z.number().min(0).max(100).optional().nullable(),

    min_months: z.number().int().positive().default(12),
    max_months: z.number().int().positive("Must be > 0"),
    min_down_payment_percent: z.number().min(0).max(100, "Must be 0–100"),
...
export const ruleSchema = z.object({
    ...
    field: z.enum(["age", "salary", "price", "car_age", "job_type", "owns_property", "salary_transfer", "down_payment", "customer_type", "employment_type", "business_age"]),
    operator: z.enum(["<", ">", "<=", ">=", "=", "!="]),
    value: z.number(),
